...
- Successful installs also persist the snippet source under `session_workspace/snippets/...` for later audit, but that archive is not auto-replayed.
...
- Treat `session_history_root` from `frida://service/config` as the durable archive root for user-facing MCP session records.
...
- Quick generator keeps capability preloads alive through explicit original binding references in that generated `index.ts`.
- Quick path assumes both `frida-compile` and `npm` are already installed in the MCP environment `PATH`.
- MCP startup warms the shared toolchain before serving stdio; a missing tool or failed compile probe aborts startup instead of deferring the error to the first `session_open_quick`.
- Quick path does not `npm install frida-compile`, `frida`, or `@types/node` into each prepared workspace.
- The shared npm cache lives under `prepared_cache_root/npm-cache`, and the shared runtime toolchain lives under `prepared_cache_root/_toolchains/<digest>`.
- `prepared_cache_root` is not the main user archive. Each opened MCP session gets a dedicated `{yyyyMMdd-HHMMSS-shortid}` directory under `session_history_root`.
...
| `dex_probe` | `DexTools` | `DexTools.enumerateClassLoaderDexFiles().length`, `DexTools.dumpAllDex("case-001")` | Also brings the JNI/ART helpers needed by dex flows. |
...
| `dex` | `DexTools` | `DexTools.dumpAllDex("case-001")` | Also makes the ART binding available transitively. |
