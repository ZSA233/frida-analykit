...
- `session_open(config_path, ...)` is a low-level tool after MCP has started successfully; if MCP itself failed to boot, fix quick-path warmup first instead of treating `session_open` as a startup bypass.
- Treat the generated quick `index.ts` as the effective compile entry when you need to inspect what MCP actually built.
- Every real MCP session gets a dedicated archive directory named `{yyyyMMdd-HHMMSS-shortid}`. Read `session_status` or `frida://session/current` to find it.
...
- `bootstrap_path` remains a separate import in the generated `index.ts`, but quick path only copies that one file.
...
- Successful installs also persist the snippet source under the current session archive directory for later inspection.
...
- Use `session_workspace` from `frida://session/current` to inspect the durable copied workspace and archived snippet files on disk.
...
- Recover reuses the same session archive directory and appends new events instead of creating a new one.
