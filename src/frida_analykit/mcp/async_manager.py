...
class AsyncDebugSessionManager:
    def __init__(
        self,
        *,
    ...
    async def session_open(
        ...
        *,
        ...
        async with self._lock:
            ...
            self._current = await self._open_active_session(spec)
    ...
    async def _open_active_session(
        ...
        *,
        ...
        try:
            ...
            script = await _to_thread(session.create_script, source)
    ...
    async def _snapshot_handle_async(self, handle: JsHandle, *, include_props: bool) -> HandleSnapshot:
    ...
    @staticmethod
    async def _resolve_snippet_target_async(handle: JsHandle | None, method: str | None) -> JsHandle:
        if handle is None:
    ...
    def _bind_home_loop(self) -> None:
        ...
        if self._home_loop is not loop:
            raise MCPManagerError("AsyncDebugSessionManager must be used from a single event loop")

    def _ensure_not_closed(self) -> None:
        if self._closed:
            raise MCPManagerError("AsyncDebugSessionManager is closed")
    ...
    def _to_json_compatible(value: Any) -> Any:
        ...
        if isinstance(value, (list, tuple)):
            return [AsyncDebugSessionManager._to_json_compatible(item) for item in value]
        if isinstance(value, dict):
            return {str(key): AsyncDebugSessionManager._to_json_compatible(item) for key, item in value.items()}
        return str(value)
