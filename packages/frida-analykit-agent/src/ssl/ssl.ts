...
import { Libssl } from "../native/libssl/libssl.js"
import { FuncHelper } from "./internal/callbacks.js"
...
export class BoringSSL {
    ...
    scanKeylogFunc(
        ...
        // 特征string 扫描
        const stringTargets = help.mem.scan(
...
class SSLTools extends NativePointerObject {
    ...
    static attachLibsslKeylogFunc(tag: string = 'sslkey.log'){
        ...
        Interceptor.attach(handle,{
            onEnter(args) {
                const [ctx] = unwrapArgs(args, 1) as NativePointer[]
                this.ssl_ctx = ctx
            },
            onLeave(retval){
    ...
    Handler(): ScriptInvocationListenerCallbacks {
        ...
        return {
            onEnter(args: InvocationArguments): void {
                const [ssl, label, secret, secret_len] = unwrapArgs(args, 4) as NativePointer[]
                const handle = new SSL(ssl)
                const len = secret_len.toUInt32()
                const secretBytes = secret.readByteArray(len) || new ArrayBuffer(0)
                const data = handle.secretlog(label.readCString() || "", secretBytes, len)
